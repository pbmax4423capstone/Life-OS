...
// All account types that require payment tracking. Includes both current and
// legacy DB values so existing rows don't lose their payment details.
const DEBT_TYPES = [
  'credit_card', 'mortgage', 'auto_loan', 'loan',
  'student_loan', 'personal_loan', 'bnpl', '401k_loan', 'heloc',
]
...
function categoryBadge(t: string): { label: string; cls: string } {
  if (['loan', 'auto_loan', 'mortgage', 'student_loan', 'personal_loan', 'heloc', '401k_loan'].includes(t))
...
function typeBadge(t: string) {
  const label = DB_TO_LABEL[t] ?? t.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())
  if (['credit_card', 'bnpl'].includes(t))
...
export default function AccountsPage() {
  ...
  const showDebtFields = DEBT_TYPES.includes(formTypeKey)
  ...
  const save = async () => {
    ...
    try {
      ...
      // Save or update scheduled payment for debt accounts
