...
const DB_TYPE_MAP: Record<string, string> = {
  ...
  '401k_loan':          'other',
  auto_loan:            'loan',   // DB enum uses 'loan', not 'auto_loan'
}
    ...
    try {
      ...
      if (isBNPL && accountId) {
        ...
        if (form.bnpl_payment_amount && form.bnpl_due_date) {
          ...
          if (existing) {
            Promise.resolve(supabase.from('scheduled_payments').update(pmtPayload).eq('id', existing.id).select('*').single())
              .then(({ data: updated }) => { if (updated) setScheduledPayments(p => p.map(x => x.id === updated.id ? updated : x)) })
              .catch(() => {})
          } else {
            Promise.resolve(supabase.from('scheduled_payments').insert(pmtPayload).select('*').single())
