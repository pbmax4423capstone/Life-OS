...
function parseOptionalNumber(value: string): number | null {
  ...
  if (!Number.isFinite(parsed)) throw new Error('Numeric field must be a valid number.')
  return parsed
}

function parseRequiredNumber(value: string, fieldName: string): number {
  const parsed = parseOptionalNumber(value)
  if (parsed === null) throw new Error(`${fieldName} is required.`)
  return parsed
}
...
export default function TravelPage() {
  ...
  const programMutation = useMutation({ mutationFn: async () => { const payload = { user_id: userId, program_name: programForm.program_name, program_type: programForm.program_type, account_number: programForm.account_number || null, points_balance: parseRequiredNumber(programForm.points_balance, 'Points balance'), value_per_point: parseRequiredNumber(programForm.value_per_point, 'Value per point'), tier_status: programForm.tier_status || null, expiration_date: programForm.expiration_date || null }; if (editProgramId) { const { error } = await db.from('loyalty_programs').update(payload).eq('id', editProgramId).eq('user_id', userId as string); if (error) throw new Error(error.message) } else { const { error } = await db.from('loyalty_programs').insert(payload); if (error) throw new Error(error.message) } }, onSuccess: async () => { toast.success(editProgramId ? 'Program updated.' : 'Program added.'); setProgramModalOpen(false); setEditProgramId(null); setProgramForm({ program_name: '', program_type: 'airline_miles', account_number: '', points_balance: '', value_per_point: '', tier_status: '', expiration_date: '' }); await invalidate() }, onError: (e: Error) => toast.error(e.message) })
  const deleteProgramMutation = useMutation({ mutationFn: async (id: string) => { const { error } = await db.from('loyalty_programs').update({ is_deleted: true }).eq('id', id).eq('user_id', userId as string); if (error) throw new Error(error.message) }, onSuccess: async () => { toast.success('Program deleted.'); await invalidate() }, onError: (e: Error) => toast.error(e.message) })
  const txMutation = useMutation({ mutationFn: async () => { const { error } = await db.from('loyalty_transactions').insert({ program_id: txModal.program_id, transaction_type: txModal.transaction_type, points_amount: parseRequiredNumber(txModal.points_amount, 'Points amount'), cash_value: parseOptionalNumber(txModal.cash_value), description: txModal.description || null, transaction_date: txModal.transaction_date || null }); if (error) throw new Error(error.message) }, onSuccess: async () => { toast.success('Transaction added.'); setTxModal({ open: false, program_id: '', transaction_type: 'earn', points_amount: '', cash_value: '', description: '', transaction_date: '' }); await invalidate() }, onError: (e: Error) => toast.error(e.message) })

  const flightMutation = useMutation({ mutationFn: async () => { const payload = { user_id: userId, airline: flightForm.airline, flight_number: flightForm.flight_number, origin_airport: flightForm.origin_airport, destination_airport: flightForm.destination_airport, departure_datetime: flightForm.departure_datetime, arrival_datetime: flightForm.arrival_datetime || null, seat_number: flightForm.seat_number || null, cabin_class: flightForm.cabin_class, confirmation_number: flightForm.confirmation_number || null, household_member_id: flightForm.household_member_id || null, loyalty_program_id: flightForm.loyalty_program_id || null, status: flightForm.status, notes: flightForm.notes || null }; if (editFlightId) { const { error } = await db.from('flights').update(payload).eq('id', editFlightId).eq('user_id', userId as string); if (error) throw new Error(error.message) } else { const { error } = await db.from('flights').insert(payload); if (error) throw new Error(error.message) } }, onSuccess: async () => { toast.success(editFlightId ? 'Flight updated.' : 'Flight added.'); setFlightModalOpen(false); setEditFlightId(null); setFlightForm({ airline: '', flight_number: '', origin_airport: '', destination_airport: '', departure_datetime: '', arrival_datetime: '', seat_number: '', cabin_class: 'economy', confirmation_number: '', household_member_id: '', loyalty_program_id: '', status: 'scheduled', notes: '' }); await invalidate() }, onError: (e: Error) => toast.error(e.message) })
  const deleteFlightMutation = useMutation({ mutationFn: async (id: string) => { const { error } = await db.from('flights').update({ is_deleted: true }).eq('id', id).eq('user_id', userId as string); if (error) throw new Error(error.message) }, onSuccess: async () => { toast.success('Flight deleted.'); await invalidate() }, onError: (e: Error) => toast.error(e.message) })

  const docMutation = useMutation({ mutationFn: async () => { const payload = { user_id: userId, household_member_id: docForm.household_member_id || null, document_type: docForm.document_type, document_number: docForm.document_number, country_of_issue: docForm.country_of_issue, issue_date: docForm.issue_date || null, expiry_date: docForm.expiry_date || null }; if (editDocId) { const { error } = await db.from('travel_documents').update(payload).eq('id', editDocId).eq('user_id', userId as string); if (error) throw new Error(error.message) } else { const { error } = await db.from('travel_documents').insert(payload); if (error) throw new Error(error.message) } }, onSuccess: async () => { toast.success(editDocId ? 'Document updated.' : 'Document added.'); setDocModalOpen(false); setEditDocId(null); setDocForm({ document_type: 'passport', document_number: '', country_of_issue: '', issue_date: '', expiry_date: '', household_member_id: '' }); await invalidate() }, onError: (e: Error) => toast.error(e.message) })
  const deleteDocMutation = useMutation({ mutationFn: async (id: string) => { const { error } = await db.from('travel_documents').update({ is_deleted: true }).eq('id', id).eq('user_id', userId as string); if (error) throw new Error(error.message) }, onSuccess: async () => { toast.success('Document deleted.'); await invalidate() }, onError: (e: Error) => toast.error(e.message) })
